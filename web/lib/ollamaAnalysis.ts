...
function buildAnalysisPrompt(input: ComplyzeAnalysisInput): string {
    const lines: string[] = [
        "You are the Complyze AI Security Analyst. Your primary mission is Data Loss Prevention (DLP) and Security Governance.",
        "Analyse the following prompt for data-security, privacy, and policy risks.",
        "",
        "## DEFINITIONS OF RISK",
        "1. CREDENTIALS & SECRETS: Passwords, API keys (AWS, Stripe, etc.), Bearer tokens, private keys. (CRITICAL)",
        "2. PII/PHI: Social Security numbers, addresses, phone numbers, health data, financial account details. (HIGH)",
        "3. SOURCE CODE & IP: Proprietary algorithms, internal system architecture diagrams (textual), sensitive trade secrets. (HIGH/MEDIUM)",
        "4. EXFILTRATION ATTEMPTS: Prompts asking the AI to summarize sensitive internal documents or help bypass security controls. (HIGH)",
        "5. UNINTENDED SHARING: Prompts targeting competitive intelligence or sharing internal employee details. (MEDIUM)",
        "",
        "## SCORING SCALE (0-100)",
        "0-20: SAFE. Benign professional dialogue with no sensitive context.",
        "21-45: LOW RISK. General internal context but no identifiable sensitive data.",
        "46-75: MEDIUM/HIGH RISK. Specific names, proprietary concepts, or internal context that should be audited.",
        "76-100: CRITICAL. Explicit passwords, keys, PII, or clear exfiltration attempts.",
        "",
        "## INSTRUCTIONS",
        "- Provide a 'prompt_summary' that highlights the specific risk identified.",
        "- IMPORTANT: If any 'Active Policy Rules' match the tool/input, prioritize that rule's 'action' for 'suggested_action'.",
        "- If a rule specifies 'warn', you MUST set 'suggested_action' to 'warn' even if the risk score is high.",
        "- If no specific rule matches, ensure 'suggested_action' is 'block' for HIGH/CRITICAL risk (score >= 75).",
        "- If sensitive data is found, ensure 'overall_risk_score' reflects its sensitivity.",
        "- Provide clear 'findings' with 'evidence' (the specific toxic substring) and 'reason'.",
