    ...
    const mitmServer = http.createServer(async (req, res) => {
        ...
        try {
            ...
            req.on('end', async () => {
                ...
                // DLP Inspection (async, non-blocking — errors are swallowed)
                let dlpResult = null;
                try {
                ...
                // Log asynchronously — never let logging delay forwarding
                logToComplyze(`https://${hostname}${reqPath}`, method, req.headers, bodyStr.substring(0, 2000), dlpResult).catch(() => { });

                // Only block if enforcement mode is ON and DLP says BLOCK
