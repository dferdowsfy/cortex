            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1">
                ...
                <section className="lg:col-span-5 card flex flex-col items-center justify-center relative overflow-hidden group shadow-2xl border-none ring-1 ring-[var(--border-main)] py-12">
                    ...
                    <header className="mb-10 text-center">
                        <h2 className="text-[15px] font-black text-[var(--text-primary)] uppercase tracking-[0.4em] mb-2 font-mono">Current AI Risk</h2>
                    </header>
                    <div className="relative">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                            <motion.span
                                ...
                                className="text-[110px] font-black italic tracking-tighter tabular-nums text-[var(--text-primary)] leading-none"
                            >
                                {riskScore}
                            </motion.span>
                        </div>
                    ...
                    <div className="mt-12 text-center w-full px-8">
                        <div className={`mb-6 px-10 py-3 rounded-xl text-[14px] font-black uppercase tracking-[0.2em] shadow-xl ${riskTheme.badge} border-none`}>
                            {riskTheme.label} Risk Posture
                        </div>

                        {/* Simple SVG Trend Visual */}
                        <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                            <div className="flex justify-between items-center mb-2 px-1">
                                <span className="text-[11px] font-bold text-white/30 uppercase tracking-widest">30D Exposure Flux</span>
                                <span className="text-[12px] font-black text-emerald-500 uppercase tracking-tighter">↓ 12% Improved</span>
                            </div>
                            <svg viewBox="0 0 200 60" className="w-full h-12 overflow-visible">
                                <path
                                    d="M0 50 Q 25 45, 50 30 T 100 35 T 150 15 T 200 25"
                                    fill="none"
                                    stroke="url(#gradient)"
                                    strokeWidth="3"
                                    strokeLinecap="round"
                ...
                <div className="lg:col-span-7 flex flex-col gap-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <MinimalMetricCard title="Extensions Active" value={browsersProtected} />
                        ...
                        {metricsAreZero ? (
                            ...
                            <div className="flex-1 flex flex-col overflow-hidden">
                                <div className="flex justify-between items-center mb-8">
                                    <h3 className="text-[13px] font-bold tracking-[0.2em] text-muted uppercase font-mono">
                                    ...
                                    {(recentActivity || []).slice(0, 5).map((event) => (
                                        <div key={event.id} className="flex items-center justify-between group/item py-2 border-b border-[var(--border-soft)] last:border-0 pb-4">
                                            <div className="flex flex-col">
                                                <span className="text-base font-bold text-[var(--text-primary)] uppercase tracking-tight">{event.tool_domain?.split('.')[0] || "Unknown Tool"}</span>
                                                <span className="text-[13px] text-[var(--text-secondary)] font-bold uppercase tracking-wider mt-0.5">{event.risk_category || "General Activity"}</span>
                                            </div>
                                            <div className="text-right">
...
function MinimalMetricCard({ title, value }: { title: string; value: string | number }) {
    return (
        <div className="card text-center p-8 transition-transform hover:scale-[1.02]">
            <p className="text-[12px] font-black text-muted uppercase tracking-[0.2em] mb-4 font-mono">{title}</p>
            <p className="text-5xl font-black text-primary italic tracking-tighter tabular-nums">{value}</p>
        </div>
    );
}
