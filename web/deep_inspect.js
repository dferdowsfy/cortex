...
async function deepInspect() {
    ...
    console.log('--- Deep Inspecting Non-Empty Policies ---');
    for (const [id, org] of Object.entries(orgs)) {
        if (Object.keys(org.policy_config || {}).length > 0 || org.policy_version > 0) {
            console.log(`Org: ${org.name} (${id})`);
            console.log(JSON.stringify(org.policy_config, null, 2));
        }
    }
    
    // Also check for a top-level 'policies' or 'rules' path if it exists
    const policiesSnap = await db.ref('policies').get();
    if (policiesSnap.exists()) {
        console.log('--- Top Level policies path found! ---');
        console.log(JSON.stringify(policiesSnap.val(), null, 2));
    }
