...
class ProxyController {
    ...
    getActiveInterface() {
        try {
            ...
            const services = execSync("networksetup -listallnetworkservices", { encoding: 'utf8' }).split('\n');
            for (const service of services) {
                if (!service || service.includes('*')) continue;
                try {
                    const dev = execSync(`networksetup -gethwportvar "${service}" | grep Device | awk '{print $2}'`, { encoding: 'utf8' }).trim();
                    if (dev === activeIf) return service;
                } catch (e) { }
            }
            return "Wi-Fi";
        } catch (e) { return "Wi-Fi"; }
    }
